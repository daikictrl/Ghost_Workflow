...
var liveblocks = globalForLiveblocks.liveblocks ?? new Liveblocks({
  secret: process.env.LIVEBLOCKS_SECRET_KEY || process.env.LIVEBLOCKS_SECRETE_KEY || "sk_mock_secret_key_for_build"
});
if (process.env.NODE_ENV !== "production") {
...
// trigger/design-agent.ts
var NodeMutationSchema = external_exports.object({
  type: external_exports.enum(["add", "update", "delete"]),
  id: external_exports.string().describe("Unique ID for the node. E.g. 'node_1', 'user_svc', etc."),
  position: external_exports.object({
    x: external_exports.number(),
    y: external_exports.number()
  }).optional().describe("X and Y coordinates on the canvas."),
  label: external_exports.string().optional().describe("Display text label of the node."),
  shape: external_exports.enum(NODE_SHAPES).optional().describe("Shape of the node."),
  color: external_exports.string().optional().describe("Hex fill color of the node. Must be one of: #1F1F1F, #10233D, #2E1938, #331B00, #3C1618, #3A1726, #0F2E18, #062822"),
  width: external_exports.number().optional().describe("Width of the node. Must follow the standard size for the chosen shape (e.g., rectangle: 180, diamond: 110, circle: 90, pill: 140, cylinder: 110, hexagon: 130)."),
  height: external_exports.number().optional().describe("Height of the node. Must follow the standard size for the chosen shape (e.g., rectangle: 80, diamond: 110, circle: 90, pill: 60, cylinder: 110, hexagon: 110).")
});
var EdgeMutationSchema = external_exports.object({
  type: external_exports.enum(["add", "delete"]),
  id: external_exports.string().describe("Unique ID for the edge. E.g. 'edge_1' or 'node1-node2'"),
  source: external_exports.string().optional().describe("Source node ID"),
  target: external_exports.string().optional().describe("Target node ID"),
  sourceHandle: external_exports.string().optional().describe("Handle ID on source node. E.g. 'right-source', 'bottom-source', 'left-source', 'top-source'"),
  targetHandle: external_exports.string().optional().describe("Handle ID on target node. E.g. 'left-target', 'top-target', 'right-target', 'bottom-target'"),
  label: external_exports.string().optional().describe("Short edge label (1-3 words MAX). Describes the protocol or relationship, e.g. 'REST', 'gRPC', 'Pub/Sub', 'Reads', 'Writes'. NEVER repeat the node name here."),
  labelOffsetPct: external_exports.number().min(0.15).max(0.85).optional().describe("Where to position the label along the edge path as a percentage (0.0 = source end, 1.0 = target end, 0.5 = midpoint). Vary this across edges sharing similar routes to avoid label overlap. Default is 0.5.")
});
var GeminiOutputSchema = external_exports.object({
  explanation: external_exports.string().describe("A brief, friendly recap of what you created, modified, or deleted, matching the user's request."),
  nodes: external_exports.array(NodeMutationSchema),
  edges: external_exports.array(EdgeMutationSchema)
});
var delay = /* @__PURE__ */ __name((ms) => new Promise((resolve) => setTimeout(resolve, ms)), "delay");
async function pushStatus(roomId, text, status) {
  const secretKey = process.env.LIVEBLOCKS_SECRET_KEY || process.env.LIVEBLOCKS_SECRETE_KEY;
  if (!secretKey) return;
  const msg = {
    text,
    status,
...
async function updatePresence(roomId, thinking, cursor) {
  const secretKey = process.env.LIVEBLOCKS_SECRET_KEY || process.env.LIVEBLOCKS_SECRETE_KEY;
  if (!secretKey) return;
  try {
...
async function patchStorage(roomId, operations) {
  const secretKey = process.env.LIVEBLOCKS_SECRET_KEY || process.env.LIVEBLOCKS_SECRETE_KEY;
...
__name(patchStorage, "patchStorage");
async function pushChatMessage(roomId, content) {
  const secretKey = process.env.LIVEBLOCKS_SECRET_KEY || process.env.LIVEBLOCKS_SECRETE_KEY;
  if (!secretKey) return;
  const msg = {
...
__name(pushChatMessage, "pushChatMessage");
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      const geminiResult = await runWithFailover(async (model) => {
        const promptString = `You are a professional system architecture designer who is an expert in building distributed systems and software architecture with over 20 years of experience. YOunamed Ghost AI.
...
- Set "labelOffsetPct" to stagger labels along the path:
  - First edge between two areas: 0.5 (midpoint)
  - Second edge in a similar corridor: 0.3 or 0.7
  - Third edge: 0.2 or 0.8
  This ensures labels never pile up at the same position even if paths are close.
        ...
        await patchStorage(roomId, [
          {
            ...
            value: {
              ...
              data: {
                label,
                labelOffsetPct: e.labelOffsetPct ?? 0.5
              }
            }
          }
        ]);
